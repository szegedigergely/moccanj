...
(function($, window, document) {
    "use strict";
    var gm, services = {}, loadOptions, // Proxify functions to get shorter minimized code
    when = $.when, extend = $.extend, isArray = $.isArray, isFunction = $.isFunction, deferred = $.Deferred;
    /**
   ...
   */
    function slice() {
        var fn = Array.prototype.slice, args = fn.call(arguments, 1);
   ...
   **/
    function distanceInMeter(origin, destination) {
        var m = Math, pi = m.PI, e = pi * origin.lat() / 180, f = pi * origin.lng() / 180, g = pi * destination.lat() / 180, h = pi * destination.lng() / 180, cos = m.cos, sin = m.sin;
        return 1e3 * 6371 * m.acos(m.min(cos(e) * cos(g) * cos(f) * cos(h) + cos(e) * sin(f) * cos(g) * sin(h) + sin(e) * sin(g), 1));
    }
    function ready(fn) {
    ...
    when(function() {
        var dfd = deferred(), cbName = "__gmap3", script;
        $.holdReady(true);
        ready(function() {
   ...
   */
    function Cluster(map, options) {
        var timer, igniter, previousViewHash, projection, filter, self = this, markers = [], radius = (options.size || 200) >> 1, enabled = true, overlays = {}, handlers = [];
        options = options || {};
     ...
     */
        function extendsMapBounds() {
            var circle = gmElement("Circle", {
                ...
                radius: 1.15 * distanceInMeter(map.getCenter(), map.getBounds().getNorthEast())
            });
     ...
     */
        function redraw() {
            var keys, bounds, overlayOptions, hash, currentMarkers, viewHash, zoom = map.getZoom(), currentHashes = {}, newOverlays = [], ignore = {};
            viewHash = "" + zoom;
            if (zoom > 3) {
   ...
   */
    $.fn.gmap3 = function(options) {
        ...
        // once gmap3 is loaded, google.maps library should be loaded
        this.each(function() {
   ...
   */
    function Handler(chain, items) {
        ...
        objectKeys(items[0]).forEach(function(name) {
            self[name] = function() {
                var results = [], args = slice(arguments);
   ...
   */
    function Gmap3($container, options) {
        var map, previousResults = [], promise = when(), self = this;
        function context() {
            return {
                $: $container,
                get: self.get
                    ...
                    gm.event["add" + (isDom ? "Dom" : "") + "Listener" + (once ? "Once" : "")](eventListener, eventName, function(event) {
                        foreach(handlers, function(handler) {
                            if (isFunction(handler)) {
                                if (isClusterOverlay) {
                                    handler.call(context(), undefined, instance, instance.cluster, event);
                                } else if (hasArgs) {
